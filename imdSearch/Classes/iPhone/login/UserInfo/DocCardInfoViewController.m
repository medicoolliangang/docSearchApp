...
@interface DocCardInfoViewController (){
    ASIHTTPRequest *docRequest;
}

@end

@implementation DocCardInfoViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.navigationItem.title = @"绑定文献卡";
    }
    return self;
}
...
- (IBAction)nextBtnClick:(id)sender{
    ...
    if (docNum == nil || [docNum isEqualToString:@""]) {
        ...
        [UrlRequest setToken:docRequest];
...
- (void)requestFailed:(ASIHTTPRequest *)request{
  ...
  if ([request.error code] == ASIRequestTimedOutErrorType) {
    [self showAlertViewWithTitle:HINT_TEXT message:REQUEST_TIMEOUT_MESSAGE];
  }else
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    self.docCardBgImg.image = [UIImage imageNamed:@"img-typin-sigle-active"];
    
    return YES;
}

- (BOOL)textFieldShouldEndEditing:(UITextField *)textField{
    self.docCardBgImg.image = [UIImage imageNamed:@"img-typin-sigle-default"];
