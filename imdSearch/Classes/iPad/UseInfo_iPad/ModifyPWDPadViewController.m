...
@interface ModifyPWDPadViewController ()

@end

@implementation ModifyPWDPadViewController
...
- (IBAction)savePassword:(id)sender{
    ...
    if (![password isEqualToString:rePassword]) {
        ...
        [self.repasswordInfo becomeFirstResponder];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (alertView.tag == 0) {
        ...
        for (UIViewController *viewController in viewControllerArr) {
            if ([viewController isKindOfClass:[FindPWDAccountInfoPadViewController class]]) {
                [self.navigationController popToViewController:viewController animated:YES];
            }
        }
    }
