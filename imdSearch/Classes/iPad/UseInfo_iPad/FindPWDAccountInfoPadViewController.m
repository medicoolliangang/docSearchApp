//
//  FindPWDAccountInfoPadViewController.m
//  imdSearch
//
...
#define TagUIAlertView  131031011

@interface FindPWDAccountInfoPadViewController (){
    ASIHTTPRequest *request;
}

@end

@implementation FindPWDAccountInfoPadViewController
...
- (void)viewDidLoad
{
    ...
    if (self.userAccount != nil && !self.isActive) {
...
#define CHECK_EMAL @"checkemail"
        ...
        request = [UrlRequest sendPadWithTokenWithUserInfo:[ImdUrlPath checkEmailUrl:self.accountInfolabel.text]  userInfo:userInfo delegate:self];
    ...
    switch (self.type) {
        case ViewTypeFindPWD:
        {
            if ([responseStr isEqualToString:@"true"]) {
                if (request_.tag == 1) {
                    //跳转
                    VerifyCheckPadViewController *viewController = [[VerifyCheckPadViewController alloc] init];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (alertView.tag == 2) {
        [self.navigationController popViewControllerAnimated:YES];
    }else if (alertView.tag == 13102401 || alertView.tag == TagUIAlertView){
        if (buttonIndex == 0) {
    ...
    }else if (alertView.tag == 2014052901 && buttonIndex == 0){
        self.accountInfolabel.text = nil;
        [self.accountInfolabel becomeFirstResponder];
    }
...
#pragma mark - check user account is exit
...
- (void)checkMobileInfoRequestFinished:(ASIHTTPRequest *)request_{
    ...
    if (self.type == ViewTypeFindPWD) {
        ...
        BOOL isActive = [self.accountInfolabel.text isEqualToString:loginNumber];
      if (!isActive) {
        isActive = [self.accountInfolabel.text isEqualToString:self.userAccount];
      }
        if (!checked && !isActive) {
